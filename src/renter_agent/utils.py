...
async def create_google_maps_tools():
    """
    Create tools from the Google Maps Review Scraper MCP server.
    
    Returns:
        List of LangChain tools for interacting with Google Maps reviews.
    """
    try:
        # Path to your MCP server
        mcp_server_path = os.path.join(os.path.dirname(__file__), "..", "tools", "google-maps-review-scraper", "index.ts")
        ...
        print(f"Warning: Could not create Google Maps MCP tools: {e}")
        return []


def create_google_maps_tools_sync():
    """
    Synchronous wrapper for creating Google Maps tools.
    ...
    import asyncio
    try:
        return asyncio.run(create_google_maps_tools())
    except Exception as e:
        print(f"Warning: Could not create Google Maps MCP tools: {e}")
        return []
